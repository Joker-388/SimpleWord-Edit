...
@interface JKREditViewController ()<UITextViewDelegate, LMSegmentedControlDelegate, LMStyleSettingsControllerDelegate, LMImageSettingsControllerDelegate>
...
- (void)getNetworkHTMLStringSuccess:(void (^)(NSString *))success failure:(void (^)(NSError *))failure {
    __block NSString *str = self.exportHTML;
    NSArray<NSTextCheckingResult *> *imageFileURLResults = [[LMHTMLHelper regexImage] matchesInString:str options:kNilOptions range:NSMakeRange(0, str.length)];
    
    NSMutableArray<NSValue *> *ranges = [NSMutableArray array];
    NSMutableArray<NSData *> *datas = [NSMutableArray array];
    
    for (NSTextCheckingResult *fileURL in imageFileURLResults) {
        NSRange range = fileURL.range;
        NSString *string = [str substringWithRange:range];
        NSString *imagePath = [string substringWithRange:NSMakeRange(10, string.length - 17)];
        NSData *data = [NSData dataWithContentsOfURL:[NSURL URLWithString:imagePath]];
        UIImage *image = [UIImage imageWithData:data];
        if (image == nil) {
            continue;
        }
        CGSize newSize = CGSizeMake(300, 300 * (image.size.height / image.size.width));
    ...
    [[JKRHTTPClient sharedClient] POST:@"api/common/upload_file" parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        for (NSData *data in datas) {
            [formData appendPartWithFileData:data name:@"files" fileName:@"image.jpeg" mimeType:@"image/jpeg"];
        }
    } progress:^(NSProgress * _Nonnull uploadProgress) {
        CGFloat progress = 100.0 * uploadProgress.completedUnitCount / uploadProgress.totalUnitCount;
        NSLog(@"=====> %.2lf%%", progress);
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSInteger locationClip = 0;
        NSArray<NSString *> *imageURLs = responseObject[@"data"];
        for (unsigned int i = 0; i < ranges.count; i++) {
            NSString *imageHtml = [NSString stringWithFormat:@"<img src=\"%@\" width", imageURLs[i]];
            
            NSLog(@"字符串长度： %zd", str.length);
            NSLog(@"便宜量： %zd", locationClip);
            NSLog(@"字符串替换位置： %zd", ranges[i].rangeValue.location + locationClip);
            NSLog(@"字符串替换长度： %zd", ranges[i].rangeValue.length);
            
            str = [str stringByReplacingCharactersInRange:NSMakeRange(ranges[i].rangeValue.location + locationClip, ranges[i].rangeValue.length) withString:imageHtml];
            locationClip += imageHtml.length - ranges[i].rangeValue.length;
        }
        success(str);
